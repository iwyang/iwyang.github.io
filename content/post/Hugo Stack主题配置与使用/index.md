...
没问题！经历了前面的踩坑，你现在已经掌握了底层原理。这次我们把所有经验融会贯通，制定一份**“绝对避坑、零报错、完美中文显示”**的终极保姆级执行手册。
...
因为你之前的 Token 已经在视频中暴露，为了你的仓库安全，**请务必先做这一步**：

1. 登录 GitHub ➔ `Settings` ➔ `Developer settings` ➔ `Personal access tokens (classic)`。
2. 找到旧的 Token，点击 **Delete** 彻底删除。
3. 点击 **Generate new token (classic)**，勾选 **`repo`** 权限，生成一个新的。
4. **复制新 Token**，保存在手机备忘录里备用（不要发出来）。

---

### 🛠️ 第二步：建立 5 个变量（杜绝乱码与路径报错）

打开 HTTP Shortcuts App，点击底部中间的 **`{ }`** 图标。我们需要建 5个输入变量。
...
点击 **脚本编写** ➔ **在执行前运行 JavaScript**，**完整粘贴**你提供的这段“手动挡”无敌代码：
...
* 最终看起来应该是：`.../shuoshuo/{DIR_NAME}/index.md`（确保 `{DIR_NAME}` 是被识别的高亮变量块，绝不能是 `%7B...`）。

#### 3. 配置认证（请求头）

点击 **请求头** ➔ 点击 **+**：

* **第一行** ➔ 键：`Authorization` | 值：`Bearer 你的新Token`（注意 Bearer 后有空格）。
...
* **正文**：严格按照下面的格式敲。输入 `{BASE64_CONTENT}` 时，**务必点击右侧的 `{ }` 按钮插入**，不能手动打括号！
...
4. 如果你看到手机底部弹出执行成功（或状态码 201），恭喜你，去你的 `bore.vip` 博客上刷新看看吧，完美的中文、完美的 Page Bundle 目录结构、完美的标签解析，全部搞定！
